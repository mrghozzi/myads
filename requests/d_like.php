    ...
    if(isset($_GET['id'])){
         if($_GET['t'] == "d"){
           ...
           $bn_typ = 22;
            ...
            $likenbcm = $db_con->prepare("SELECT  COUNT(id) as nbr FROM `like` WHERE sid='{$bn_id}' AND  type=22 " );
$likenbcm->execute();
$abdlike=$likenbcm->fetch(PDO::FETCH_ASSOC);
            echo "<a  href=\"javascript:void(0);\" id=\"dulike".$bn_id."\" ><i class=\"fa fa-heart\" style=\"color: #FF0000;\"  aria-hidden=\"true\"></i>{$abdlike['nbr']}</a>
            <script>
...
$likenbcm->execute();
$abdlike=$likenbcm->fetch(PDO::FETCH_ASSOC);
             echo "<a href=\"javascript:void(0);\" id=\"dlike".$bn_id."\" ><i class=\"fa fa-heart-o\" aria-hidden=\"true\"></i>{$abdlike['nbr']}</a>
               <script>
     \$(\"document\").ready(function() {
   \$(\"#dlike{$sucat['id']}\").click(postdlike{$sucat['id']});
