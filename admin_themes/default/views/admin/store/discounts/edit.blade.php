...
@section('title', __('messages.edit_discount') ?? 'Edit Discount Code')
...
<div class="pages-editor-shell">
    ...
    <div class="pages-editor-hero">
        <div>
            <ul class="pe-breadcrumb">
                ...
                <li><i class="feather-chevron-right"></i></li>
                <li>{{ __('messages.edit_discount') ?? 'Edit Discount Code' }}</li>
            </ul>
            <h2>{{ __('messages.edit_discount') ?? 'Edit Discount Code' }}</h2>
            <p>{{ __('messages.manage_store_discounts_desc') ?? 'Edit discount code settings and limits.' }}</p>
        </div>
        ...
        <div class="pe-actions">
            ...
            <button type="submit" form="discount-form" class="pe-btn-primary">
                <i class="feather-save"></i>{{ __('messages.save') ?? 'Save' }}
            </button>
        </div>
    </div>
    ...
    <form id="discount-form" action="{{ route('admin.store.discounts.update', $discount->id) }}" method="POST">
        @csrf
            ...
            <div class="pe-card-body">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $discount->name) }}" required>
                    </div>

                    <!-- Coupon Code -->
                    <div class="col-md-6 mb-4">
                        <label class="form-label" for="code">{{ __('messages.coupon_code') ?? 'Coupon Code' }} <span class="text-danger">*</span></label>
                        <input type="text" name="code" id="code" class="form-control" value="{{ old('code', $discount->code) }}" style="text-transform: uppercase;" required>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="number" name="discount_value" id="discount_value" class="form-control" value="{{ old('discount_value', $discount->discount_value) }}" min="1" required>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <select name="applies_to" id="applies_to" class="form-select" required>
                            <option value="all" {{ old('applies_to', $discount->applies_to) === 'all' ? 'selected' : '' }}>{{ __('messages.all_products') ?? 'All Products' }}</option>
                            <option value="product" {{ old('applies_to', $discount->applies_to) === 'product' ? 'selected' : '' }}>{{ __('messages.specific_product') ?? 'Specific Product' }}</option>
                            <option value="category" {{ old('applies_to', $discount->applies_to) === 'category' ? 'selected' : '' }}>{{ __('messages.product_category') ?? 'Product Category' }}</option>
                            <option value="seller" {{ old('applies_to', $discount->applies_to) === 'seller' ? 'selected' : '' }}>{{ __('messages.seller_products') ?? 'All Seller\'s Products' }}</option>
                        </select>
                    </div>

                    <!-- Target Value selectors -->
                    <!-- Product Wrapper -->
                    <div class="col-md-6 mb-4" id="product-wrapper" style="display: none;">
                        ...
                        <select id="product-select" class="form-select">
                            ...
                            @foreach($products as $prod)
                                <option value="{{ $prod->id }}" {{ $discount->applies_to === 'product' && $discount->target_value == $prod->id ? 'selected' : '' }}>{{ $prod->name }} ({{ $prod->productPrice }} PTS)</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-4" id="category-wrapper" style="display: none;">
                        ...
                        <select id="category-select" class="form-select">
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat }}" {{ $discount->applies_to === 'category' && $discount->target_value == $cat ? 'selected' : '' }}>{{ ucfirst($cat) }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-4" id="seller-wrapper" style="display: none;">
                        ...
                        <select id="seller-select" class="form-select">
                            ...
                            @foreach($sellers as $sel)
                                <option value="{{ $sel->id }}" {{ $discount->applies_to === 'seller' && $discount->target_value == $sel->id ? 'selected' : '' }}>{{ $sel->username }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="datetime-local" name="start_date" id="start_date" class="form-control" value="{{ old('start_date', $discount->start_date ? $discount->start_date->format('Y-m-d\TH:i') : '') }}">
                    </div>

                    <!-- End Date -->
                    <div class="col-md-6 mb-4">
                        <label class="form-label" for="end_date">{{ __('messages.end_date') ?? 'End Date' }}</label>
                        <input type="datetime-local" name="end_date" id="end_date" class="form-control" value="{{ old('end_date', $discount->end_date ? $discount->end_date->format('Y-m-d\TH:i') : '') }}">
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="number" name="max_uses" id="max_uses" class="form-control" value="{{ old('max_uses', $discount->max_uses) }}" min="1" placeholder="e.g. 100 (Leave blank for unlimited)">
                    </div>

                    <!-- Active Switch -->
                    <div class="col-md-6 mb-4 d-flex align-items-end">
                        <div class="form-check form-switch mb-0 w-100" style="min-height: 44px;">
                            <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $discount->is_active) ? 'checked' : '' }}>
