...
<div class="admin-page">
    ...
    <section class="admin-hero">
        <div class="admin-hero__content">
            ...
            <div class="admin-hero__eyebrow">
                <span class="badge bg-soft-primary text-primary px-2 py-1"><i class="feather-activity me-1"></i>{{ __('messages.exvisit') }}</span>
            </div>
            <h1 class="admin-hero__title d-flex align-items-center gap-2">
                <i class="feather-activity text-primary"></i>
                {{ __('messages.exvisit') }}
            </h1>
            <p class="admin-hero__copy">{{ __('messages.smart_admin_recent_inventory') }} &bull; {{ __('messages.code_unavailable') }} / {{ __('messages.Stats') }}</p>

            <!-- 4 KPI Stat Strip -->
            <div class="admin-stat-strip mt-3" id="heroStatsContainer">
                <div class="admin-stat-card">
                    <span class="admin-stat-label"><i class="feather-activity me-1 text-primary"></i>{{ __('messages.total_campaigns') }}</span>
                    <span class="admin-stat-value text-primary" id="kpiTotal">{{ number_format($stats['total'] ?? 0) }}</span>
                </div>
                <div class="admin-stat-card">
                    <span class="admin-stat-label"><i class="feather-check-circle me-1 text-success"></i>{{ __('messages.active_campaigns') }}</span>
                    <span class="admin-stat-value text-success" id="kpiActive">{{ number_format($stats['active'] ?? 0) }}</span>
                </div>
                <div class="admin-stat-card">
                    <span class="admin-stat-label"><i class="feather-pause-circle me-1 text-warning"></i>{{ __('messages.inactive_campaigns') }}</span>
                    <span class="admin-stat-value text-warning" id="kpiInactive">{{ number_format($stats['inactive'] ?? 0) }}</span>
                </div>
                <div class="admin-stat-card">
                    <span class="admin-stat-label"><i class="feather-eye me-1 text-info"></i>{{ __('messages.total_visits_delivered') }}</span>
                    <span class="admin-stat-value text-info" id="kpiDelivered">{{ number_format($stats['total_delivered'] ?? 0) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="admin-hero__actions">
            <div class="d-flex align-items-center gap-2 w-100 flex-wrap">
                ...
                <a href="{{ route('visits.create') }}" class="btn btn-primary d-inline-flex align-items-center gap-2 flex-grow-1">
    ...
    <section class="admin-panel shadow-sm">
        ...
        <div class="admin-panel__header p-3 border-bottom bg-transparent">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 w-100">
                ...
                <div class="position-relative flex-grow-1" style="max-width: 380px; min-width: 240px;">
                    <div class="input-group">
                        ...
                        <input 
                            ...
                            placeholder="{{ __('messages.search_visits_placeholder') }}" 
                ...
                <div class="d-flex flex-wrap align-items-center gap-2 ms-auto">
                    <!-- Duration Filter -->
                    <select id="durationFilter" class="form-select form-select-sm" style="width: auto; min-width: 140px;">
                        <option value="">{{ __('messages.duration') ?? 'Duration' }}: {{ __('messages.all') ?? 'All' }}</option>
                        @foreach($durationOptions as $key => $label)
                            <option value="{{ $key }}" {{ request('tims') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    ...
                    <select id="statusFilter" class="form-select form-select-sm" style="width: auto; min-width: 130px;">
                        ...
                        <option value="2" {{ request('status', $filterState['status'] ?? '') === '2' ? 'selected' : '' }}>{{ __('messages.inactive') }}</option>
                    </select>

                    <!-- Sort Filter -->
                    <select id="sortFilter" class="form-select form-select-sm" style="width: auto; min-width: 130px;">
        ...
        <!-- Table Container -->
        <div class="admin-panel__body p-0 position-relative" id="visitsTableContainer">
            @include('admin::admin.partials.visits_table', ['visits' => $visits])
        </div>
    </section>
</div>
        ...
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header border-bottom py-3">
                <h5 class="modal-title fw-bold d-flex align-items-center gap-2">
                    ...
                    {{ __('messages.edit') }} {{ __('messages.exvisit') }}
                </h5>
            ...
            <form id="editVisitForm" method="POST">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" class="form-control" id="editVisitName" name="name" required>
                        </div>
                        <div class="col-12">
                            ...
                            <div class="input-group">
                                ...
                                <input type="url" class="form-control" id="editVisitUrl" name="url" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-dark">{{ __('messages.duration') ?? 'Duration' }} <span class="text-danger">*</span></label>
                            <select class="form-select" id="editVisitTims" name="tims" required>
                                @foreach($durationOptions as $k => $label)
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="editVisitStatus" name="statu" required>
                ...
                <div class="modal-footer border-top py-3">
                    ...
                    <button type="submit" class="btn btn-primary d-inline-flex align-items-center gap-2" id="saveVisitBtn">
                        <span class="spinner-border spinner-border-sm d-none" id="saveVisitSpinner"></span>
                        <i class="feather-save" id="saveVisitIcon"></i>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body text-center py-4">
                ...
                <h5 class="fw-bold mb-2" id="deleteVisitTitle">...</h5>
            ...
            <div class="modal-footer border-0 pt-0 justify-content-center">
                ...
                <button type="button" class="btn btn-danger px-4" id="confirmDeleteVisitSubmitBtn">
                    <span class="spinner-border spinner-border-sm me-1 d-none" id="deleteVisitSpinner"></span>
                    {{ __('messages.delete') }}
                </button>
            </div>
        </div>
...
<div class="modal fade" id="bulkDeleteVisitModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title fw-bold text-danger d-flex align-items-center gap-2">
            ...
            <div class="modal-body text-center py-4">
                ...
                <h5 class="fw-bold mb-2"><span id="bulkDeleteVisitCount">0</span> {{ __('messages.selected_items') }}</h5>
                <p class="text-muted mb-0">{{ __('messages.confirm_delete_text') }}</p>
            </div>
            <div class="modal-footer border-0 pt-0 justify-content-center">
                <button type="button" class="btn btn-light px-4" data-bs-dismiss="modal">{{ __('messages.close') }}</button>
                <button type="button" class="btn btn-danger px-4" id="confirmBulkDeleteVisitSubmitBtn">
                    <span class="spinner-border spinner-border-sm me-1 d-none" id="bulkDeleteVisitSpinner"></span>
                    {{ __('messages.delete') }}
                </button>
            </div>
        </div>
...
(function() {
    ...
    const i18n = {
        ...
        visitUpdated: @json(__('messages.visit_updated_successfully')),
        visitDeleted: @json(__('messages.visit_deleted_successfully')),
    ...
    const tableContainer = document.getElementById('visitsTableContainer');
    ...
    const confirmDeleteSubmitBtn = document.getElementById('confirmDeleteVisitSubmitBtn');
    const confirmBulkDeleteSubmitBtn = document.getElementById('confirmBulkDeleteVisitSubmitBtn');
    ...
    const saveVisitIcon = document.getElementById('saveVisitIcon');
    ...
    function fetchTableData(pageUrl = null) {
        ...
        if (!pageUrl) {
            ...
            if (durationFilter.value) url.searchParams.set('tims', durationFilter.value);
        ...
        .then(data => {
            ...
            if (data.stats) {
                ...
                document.getElementById('kpiDelivered').textContent = Number(data.stats.total_delivered || 0).toLocaleString();
            }
    ...
    durationFilter.addEventListener('change', () => fetchTableData());
    ...
    tableContainer.addEventListener('change', function(e) {
        if (e.target.classList.contains('status-toggle-btn')) {
            ...
            const visitId = toggle.dataset.id;
            const newStatus = toggle.checked ? 1 : 2;
            const label = document.querySelector(`.visit-status-label-${visitId}`);

            toggle.disabled = true;

            fetch(`${currentBaseUrl}/${visitId}`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (label) {
                        ...
                        label.className = `form-check-label ms-1 small fw-semibold visit-status-label-${visitId} ${newStatus === 1 ? 'text-success' : 'text-muted'}`;
                    }
                } else {
    ...
    tableContainer.addEventListener('click', function(e) {
        ...
        if (editBtn) {
            ...
            document.getElementById('editVisitName').value = editBtn.dataset.name || '';
            document.getElementById('editVisitUrl').value = editBtn.dataset.url || '';
            document.getElementById('editVisitTims').value = editBtn.dataset.tims || '1';
            document.getElementById('editVisitStatus').value = editBtn.dataset.statu || '1';
    ...
    editVisitForm.addEventListener('submit', function(e) {
        ...
        saveVisitSpinner.classList.remove('d-none');
        saveVisitIcon.classList.add('d-none');
        ...
        .then(data => {
            ...
            saveVisitSpinner.classList.add('d-none');
            saveVisitIcon.classList.remove('d-none');
            bootstrap.Modal.getInstance(document.getElementById('editVisitModal')).hide();

            if (data.success) {
                showToast(data.message || i18n.visitUpdated, 'success');
                fetchTableData();
            } else {
                showToast(data.message || i18n.operationFailed, 'error');
            }
    ...
    tableContainer.addEventListener('click', function(e) {
        const delBtn = e.target.closest('.delete-visit-btn');
        if (delBtn) {
            deleteTargetId = delBtn.dataset.id;
            document.getElementById('deleteVisitTitle').textContent = delBtn.dataset.name || `#${deleteTargetId}`;
    ...
    confirmDeleteSubmitBtn.addEventListener('click', function() {
        ...
        const spinner = document.getElementById('deleteVisitSpinner');
        ...
        .then(data => {
            ...
            bootstrap.Modal.getInstance(document.getElementById('deleteVisitModal')).hide();

            if (data.success) {
                showToast(data.message || i18n.visitDeleted, 'success');
                const row = document.getElementById(`visitRow${deleteTargetId}`);
                if (row) row.remove();
                fetchTableData();
            } else {
                showToast(data.message || i18n.operationFailed, 'error');
    ...
    bulkDeleteTriggerBtn.addEventListener('click', function() {
        const ids = getSelectedIds();
        if (ids.length === 0) return;
        document.getElementById('bulkDeleteVisitCount').textContent = ids.length;
        const modal = new bootstrap.Modal(document.getElementById('bulkDeleteVisitModal'));
        modal.show();
    });

    confirmBulkDeleteSubmitBtn.addEventListener('click', function() {
        const spinner = document.getElementById('bulkDeleteVisitSpinner');
        spinner.classList.remove('d-none');
        this.disabled = true;
        ...
        .then(data => {
            ...
            bootstrap.Modal.getInstance(document.getElementById('bulkDeleteVisitModal')).hide();
