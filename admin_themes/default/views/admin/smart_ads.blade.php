...
<div class="admin-page">
    ...
    <section class="admin-hero">
        <div class="admin-hero__content">
            ...
            <div class="admin-stat-strip mt-3" id="heroStatsContainer">
                <div class="admin-stat-card">
                    <span class="admin-stat-label"><i class="feather-cpu me-1 text-primary"></i>{{ __('messages.total_smart_ads') }}</span>
                    <span class="admin-stat-value text-primary" id="kpiTotal">{{ number_format($stats['total'] ?? 0) }}</span>
                </div>
                <div class="admin-stat-card">
                    <span class="admin-stat-label"><i class="feather-check-circle me-1 text-success"></i>{{ __('messages.active_smart_ads') }}</span>
                    <span class="admin-stat-value text-success" id="kpiActive">{{ number_format($stats['active'] ?? 0) }}</span>
                </div>
                <div class="admin-stat-card">
                    <span class="admin-stat-label"><i class="feather-pause-circle me-1 text-warning"></i>{{ __('messages.paused_smart_ads') }}</span>
                ...
                <div class="admin-stat-card">
                    ...
                    <span class="admin-stat-value text-info" id="kpiImpressions">{{ number_format($stats['total_impressions'] ?? 0) }}</span>
                </div>
                <div class="admin-stat-card">
        ...
        <div class="admin-hero__actions">
            <div class="d-flex align-items-center gap-2 w-100 flex-wrap">
                <a href="{{ route('ads.smart.code') }}" class="btn btn-light admin-icon-btn" data-bs-toggle="tooltip" title="{{ __('messages.codes') }}">
                    <i class="feather-code"></i>
                </a>
                <a href="{{ route('admin.stats', ['ty' => 'smart']) }}" class="btn btn-light admin-icon-btn" data-bs-toggle="tooltip" title="{{ __('messages.Stats') }}">
                    <i class="feather-bar-chart-2"></i>
                </a>
                <button type="button" class="btn btn-light admin-icon-btn" id="refreshTableBtn" title="{{ __('messages.refresh') ?? 'Refresh' }}">
                    <i class="feather-refresh-cw"></i>
                </button>
                <a href="{{ route('ads.smart.create') }}" class="btn btn-primary d-inline-flex align-items-center gap-2 flex-grow-1">
    ...
    <section class="admin-panel shadow-sm">
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 w-100">
                ...
                <div class="position-relative flex-grow-1" style="max-width: 380px; min-width: 240px;">
                    <div class="input-group">
                        ...
                        <input 
                            type="text" 
                            id="smartAdSearchInput" 
                            class="form-control border-start-0 ps-0" 
                            placeholder="{{ __('messages.search_smart_ads_placeholder') }}" 
                ...
                <div class="d-flex flex-wrap align-items-center gap-2 ms-auto">
                    ...
                    <select id="statusFilter" class="form-select form-select-sm" style="width: auto; min-width: 140px;">
                        ...
                        <option value="0" {{ request('status', $filterState['status'] ?? '') === '0' ? 'selected' : '' }}>{{ __('messages.paused') }}</option>
                        <option value="2" {{ request('status', $filterState['status'] ?? '') === '2' ? 'selected' : '' }}>{{ __('messages.blocked') }}</option>
                    </select>
                    ...
                    <select id="sortFilter" class="form-select form-select-sm" style="width: auto; min-width: 140px;">
                        ...
                        <option value="impressions">{{ __('messages.sort_views') ?? 'Most Impressions' }}</option>
        ...
        <!-- Table Container -->
        <div class="admin-panel__body p-0 position-relative" id="smartAdsTableContainer">
            @include('admin::admin.partials.smart_ads_table', ['smartAds' => $smartAds])
        </div>
    </section>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body text-center py-4">
                ...
                <h5 class="fw-bold mb-2" id="deleteSmartAdTitle">...</h5>
            ...
            <div class="modal-footer border-0 pt-0 justify-content-center">
                ...
                <button type="button" class="btn btn-danger px-4" id="confirmDeleteSmartAdSubmitBtn">
                    <span class="spinner-border spinner-border-sm me-1 d-none" id="deleteSmartAdSpinner"></span>
                    {{ __('messages.delete') }}
                </button>
            </div>
        </div>
...
<div class="modal fade" id="bulkDeleteSmartAdModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title fw-bold text-danger d-flex align-items-center gap-2">
            ...
            <div class="modal-body text-center py-4">
                ...
                <h5 class="fw-bold mb-2"><span id="bulkDeleteSmartAdCount">0</span> {{ __('messages.selected_items') }}</h5>
                <p class="text-muted mb-0">{{ __('messages.confirm_delete_text') }}</p>
            </div>
            <div class="modal-footer border-0 pt-0 justify-content-center">
                <button type="button" class="btn btn-light px-4" data-bs-dismiss="modal">{{ __('messages.close') }}</button>
                <button type="button" class="btn btn-danger px-4" id="confirmBulkDeleteSmartAdSubmitBtn">
                    <span class="spinner-border spinner-border-sm me-1 d-none" id="bulkDeleteSmartAdSpinner"></span>
                    {{ __('messages.delete') }}
                </button>
            </div>
        </div>
...
(function() {
    ...
    const i18n = {
        ...
        smartAdDeleted: @json(__('messages.smart_ad_admin_deleted')),
    ...
    // DOM Elements
    const searchInput = document.getElementById('smartAdSearchInput');
    const clearSearchBtn = document.getElementById('clearSearchBtn');
    const statusFilter = document.getElementById('statusFilter');
    const sortFilter = document.getElementById('sortFilter');
    const refreshTableBtn = document.getElementById('refreshTableBtn');
    const tableContainer = document.getElementById('smartAdsTableContainer');
    ...
    const confirmDeleteSubmitBtn = document.getElementById('confirmDeleteSmartAdSubmitBtn');
    const confirmBulkDeleteSubmitBtn = document.getElementById('confirmBulkDeleteSmartAdSubmitBtn');
    ...
    function fetchTableData(pageUrl = null) {
        ...
        .then(data => {
            ...
            if (data.stats) {
                ...
                document.getElementById('kpiImpressions').textContent = Number(data.stats.total_impressions || 0).toLocaleString();
        ...
        .catch(err => {
            console.error('Fetch smart ads error:', err);
            tableContainer.style.opacity = '1';
            tableContainer.style.pointerEvents = 'auto';
    ...
    tableContainer.addEventListener('change', function(e) {
        if (e.target.classList.contains('status-toggle-btn')) {
            ...
            const adId = toggle.dataset.id;
            const newStatus = toggle.checked ? 1 : 0;
            const label = document.querySelector(`.smartad-status-label-${adId}`);

            toggle.disabled = true;

            fetch(`${currentBaseUrl}/${adId}`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (label) {
                        ...
                        label.className = `form-check-label ms-1 small fw-semibold smartad-status-label-${adId} ${newStatus === 1 ? 'text-success' : 'text-muted'}`;
                    }
                } else {
    ...
    tableContainer.addEventListener('click', function(e) {
        const delBtn = e.target.closest('.delete-smartad-btn');
        if (delBtn) {
            deleteTargetId = delBtn.dataset.id;
            document.getElementById('deleteSmartAdTitle').textContent = delBtn.dataset.name || `#${deleteTargetId}`;
            const modal = new bootstrap.Modal(document.getElementById('deleteSmartAdModal'));
            modal.show();
        }
    });
    ...
    confirmDeleteSubmitBtn.addEventListener('click', function() {
        ...
        const spinner = document.getElementById('deleteSmartAdSpinner');
        ...
        .then(data => {
            ...
            bootstrap.Modal.getInstance(document.getElementById('deleteSmartAdModal')).hide();

            if (data.success) {
                showToast(data.message || i18n.smartAdDeleted, 'success');
                const row = document.getElementById(`smartAdRow${deleteTargetId}`);
                if (row) row.remove();
                fetchTableData();
            } else {
                showToast(data.message || i18n.operationFailed, 'error');
    ...
    bulkDeleteTriggerBtn.addEventListener('click', function() {
        const ids = getSelectedIds();
        if (ids.length === 0) return;
        document.getElementById('bulkDeleteSmartAdCount').textContent = ids.length;
        const modal = new bootstrap.Modal(document.getElementById('bulkDeleteSmartAdModal'));
        modal.show();
    });

    confirmBulkDeleteSubmitBtn.addEventListener('click', function() {
        const spinner = document.getElementById('bulkDeleteSmartAdSpinner');
        spinner.classList.remove('d-none');
        this.disabled = true;
        ...
        .then(data => {
            ...
            bootstrap.Modal.getInstance(document.getElementById('bulkDeleteSmartAdModal')).hide();
