@extends('theme::layouts.master')

@section('content')
<!-- SECTION BANNER -->
<div class="section-banner" style="background: url({{ theme_asset('img/banner/Newsfeed.png') }}) no-repeat 50%;" >
    <img class="section-banner-icon" src="{{ theme_asset('img/banner/newsfeed-icon.png') }}"  alt="overview-icon">
    <p class="section-banner-title">{{ __('messages.news') }}</p>
    <p class="section-banner-text">{{ __('messages.latest_news') }}</p>
</div>

<div class="grid grid-3-9">
    <!-- LEFT SIDEBAR -->
    <div class="grid-column">
        <div class="widget-box">
            <p class="widget-box-title"><h4>{{ __('messages.menu') }}</h4></p>
            <div class="widget-box-content">
                <div class="post-peek-list">
                    <a href="{{ url('/home') }}" class="btn btn-primary" >&nbsp;<i class="fa fa-home" aria-hidden="true"></i>&nbsp;</a>
                </div>
            </div>
        </div>
    </div>

    <!-- MAIN CONTENT -->
    <div class="grid-column">
        @foreach($news as $item)
        <div class="widget-box">
            <div class="widget-box-settings">
                <div class="post-peek-header">
                    <p class="post-peek-title"><a href="{{ route('news.show', $item->id) }}">{{ $item->name }}</a></p>
                    <p class="post-peek-text">{{ $item->date ? date('Y-m-d', $item->date) : '' }}</p>
                </div>
                <div class="post-peek-body">
                    <p>{!! \Illuminate\Support\Str::limit(strip_tags($item->text), 200) !!}</p>
                </div>
            </div>
        </div>
        @endforeach

        {{ $news->links() }}
    </div>
</div>
@endsection
