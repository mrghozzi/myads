...
<div class="widget-box">
    ...
    <div class="widget-box-content padding-none">
        ...
        @if(\App\Models\Option::where('name', 'monetization')->where('o_type', 'plugins')->where('o_valuer', '1')->exists())
            <a href="{{ route('monetization.index') }}" class="button {{ request()->routeIs('monetization.*') ? 'primary' : 'secondary' }} full" style="border-radius: 0; box-shadow: none;">{{ __('monetization::messages.monetization') }}</a>
        @endif
