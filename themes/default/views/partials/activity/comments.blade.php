@foreach($comments as $comment)
        ...
        // Process Text (Links, Hashtags, RTL)
        $text = nl2br(e($text));
        ...
        $reactionTypeString = '';
        if ($type == 'forum') { $commentReactionType = 4; $reactionTypeString = 'forum_comment'; }
        elseif ($type == 'directory') { $commentReactionType = 44; $reactionTypeString = 'directory_comment'; }
        elseif ($type == 'store') { $commentReactionType = 444; $reactionTypeString = 'store_comment'; }
        ...
        if($myCommentReaction){
                $myCommentReactionOption = \App\Models\Option::where('o_parent', $myCommentReaction->id)->where('o_type', 'data_reaction')->first();
        }
    @endphp
    <!-- POST COMMENT -->
    <div class="post-comment coment{{ $comment->id }}" id="comment_{{ $comment->id }}">
        ...
        <div class="content-actions">
            <div class="content-action">
                <div class="meta-line">
                    <p class="meta-line-timestamp">{{ \Carbon\Carbon::createFromTimestamp($date)->diffForHumans() }}</p>
                </div>
