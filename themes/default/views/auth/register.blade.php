@php
    ...
    $resolvedTitle = $resolvedTitle !== '' ? $resolvedTitle : (__('messages.sign_up') . ' - ' . ($site_settings->titer ?? 'MyAds'));
@endphp
<!DOCTYPE html>
        ...
        <div class="auth-box scroll-reveal active">
            ...
            <form method="post" action="{{ route('register.post') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="password" class="form-control" id="register-password-repeat" name="password_confirmation" required>
                </div>
                ...
                <div class="form-group mb-4 mt-4">
                    <div class="checkbox-wrap" style="align-items: flex-start;">
                        <input type="checkbox" id="agree-terms" name="agree_terms" value="1" {{ old('agree_terms') ? 'checked' : '' }} required style="margin-top: 4px;">
                        <label for="agree-terms" style="font-size: 0.9rem; line-height: 1.5;">
                            {{ __('messages.agree_terms') }}
                            <a href="{{ route('privacy') }}" target="_blank" style="color: var(--primary); text-decoration: underline;">{{ __('messages.privacy_policy') }}</a>
                            {{ __('messages.and') }}
                            <a href="{{ route('terms') }}" target="_blank" style="color: var(--primary); text-decoration: underline;">{{ __('messages.terms_conditions') }}</a>
                        </label>
                    </div>
                    @error('agree_terms')
                ...
                <button class="btn-submit" name="submit" type="submit">{{ __('messages.sign_up') }}</button>
            </form>
            ...
            <div class="switch-auth">
                {{ __('messages.alrehaacc') }} <a href="{{ route('login') }}">{{ __('messages.login') }}</a>
            </div>
        </div>
    </section>
