...
if(isset($s_st) AND ($s_st=="buyfgeufb")){ 
 ...
 //  Get Browser
function tpl_store_stt($sutcat,$Suggestion)
{
...
$catuscf->execute();
$catusscf=$catuscf->fetch(PDO::FETCH_ASSOC);
$service = "store" ;
$catusc = $db_con->prepare("SELECT *  FROM options WHERE  ( o_type='{$service}' AND id='{$catusscf['o_parent']}' ) ");
$catusc->execute();
$catussc=$catusc->fetch(PDO::FETCH_ASSOC);
$comtxt = $catussc['o_valuer'];
$comtxt = strip_tags($comtxt, '<br><iframe>');
$comtxt=substr($comtxt,0,480);
$catdnb = $db_con->prepare("SELECT  COUNT(id) as nbr FROM status WHERE tp_id='{$catdid}' AND s_type=7867 " );
$catdnb->execute();
...
$linksher =  $url_site."/producer/".$catussc['name'];
...
if(isset($catussc['o_order']) AND ($catussc['o_order']>0)){
                   $storepts = $catussc['o_order']."&nbsp;<span class=\"highlighted\">PTS</span>";
                 }else{
                    $storepts = $lang['free'];
                 }
...
<?php } ?>
                <!-- SIMPLE DROPDOWN LINK -->
                <a href="<?php echo $url_site; ?>/producer/<?php echo $catussc['name']; ?>" class="simple-dropdown-link"><i class="fa fa-shopping-basket" aria-hidden="true"></i>&nbsp;<?php echo $lang['preview']; ?></a>
                <!-- /SIMPLE DROPDOWN LINK -->
...
?>
          ...
          <a href="<?php echo $url_site; ?>/producer/<?php echo $catussc['name']; ?>">
            <figure class="product-preview-image liquid" style="background: rgba(0, 0, 0, 0) url(<?php url_site();  ?>/templates/_panel/img/error_plug.png) no-repeat scroll center center / cover;">
              <img src="<?php echo $catussc['o_mode']; ?>" alt="<?php echo $catussc['name']; ?>" style="display: none;">
            </figure>
          </a>
          <!-- /PRODUCT PREVIEW IMAGE -->
          ...
          <div class="product-preview-info">
            ...
            <p class="product-preview-title"><a href="<?php echo $url_site."/producer/".$catussc['name']; ?>"><?php echo $catussc['name']; ?></a></p>
            <!-- /PRODUCT PREVIEW TITLE -->

            <!-- PRODUCT PREVIEW CATEGORY -->
            <p class="product-preview-category digital"><a href="marketplace-category.html"><?php echo $stortype; ?></a></p>
            <!-- /PRODUCT PREVIEW CATEGORY -->

            <!-- PRODUCT PREVIEW TEXT -->
            <p class="product-preview-text"><?php echo $comtxt; ?></p>
            <!-- /PRODUCT PREVIEW TEXT -->
          </div>
          <!-- /PRODUCT PREVIEW INFO -->
              ...
              <div class="content-actions">
                ...
                <div class="content-action">
                  ...
                  <div class="meta-line">
                    ...
                    <a href="<?php echo $url_site; ?>/producer/<?php echo $catussc['name']; ?>"><?php echo $abdcmnt['nbr']; ?> <?php echo $lang['comments']; ?></a>
                    </p>
                    <!-- /META LINE LINK -->
                  </div>
                  <!-- /META LINE -->
...
<?php   if(isset($_COOKIE['user'])){ ?>
            <!-- POST OPTION -->
            <div  class="post-option sh_comment_r<?php echo $sutcat['id']; ?>">
              <!-- POST OPTION ICON -->
...
<script>$('.sh_comment_r<?php echo $sutcat['id']; ?>').click(function(){
  ...
  $(".sh_comment_r<?php echo $sutcat['id']; ?>").addClass('active');
    });
</script>
<script>
     $("document").ready(function() {
   $(".reaction_7867_<?php echo $sucat['id']; ?>").click(postreaction<?php echo $sutcat['id']; ?>);
