...
<head>
    ...
    <title>{{ __('New Notification') }}</title>
    <style>
        /* Base Reset */
...
<body>
    @php
        ...
        $baseUrl = url('/');
        $fullActionUrl = $actionUrl ? (str_starts_with($actionUrl, 'http') ? $actionUrl : $baseUrl . $actionUrl) : $baseUrl;
    @endphp
    <div class="wrapper">
        <table class="main-container" width="100%" cellpadding="0" cellspacing="0" role="presentation">
            <tr>
                <td class="header">
                    <a href="{{ $baseUrl }}" class="logo-text">
                        {{ $siteName }}
                    </a>
                </td>
            </tr>
            <tr>
                <td class="content">
                    ...
                    <div class="btn-container">
                        <a href="{{ $fullActionUrl }}" class="btn">{{ __('View Details') }}</a>
                    </div>
                    
                    <p class="subtext">{{ __('You are receiving this email based on your notification settings.') }}</p>
